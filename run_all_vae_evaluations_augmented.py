...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_augmented.csv"
...
def augment_time_series(X, target_size=500):
    """
    Time series data augmentation (Jittering & Scaling) to generate diverse training samples.
    """
    N, L = X.shape
    if N >= target_size:
    ...
    for _ in range(num_augmented):
        # Pick a random sample from X
        idx = np.random.randint(0, N)
        x = X[idx].copy()
        
        # Randomly apply Jittering, Scaling, or Both
        ...
        if aug_type in ['scale', 'both']:
            # Scaling: Multiply by a scale factor between 0.9 and 1.1
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001):
    try:
        ...
        # Apply Data Augmentation instead of simple tiling
        X_train = augment_time_series(X_train, target_size=500)
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '시계열 데이터 증강(Augmented)' VAE 배치 학습 평가를 시작합니다.")
    ...
    for idx, name in enumerate(dataset_names, 1):
        ...
        if idx % 50 == 0 or idx == total_datasets:
            ...
            if valid_results:
                ...
                print(f"[{idx:4d}/{total_datasets:4d}] 진행 완료... (현재 완료된 {len(valid_results)}개 VAE(Augmented) 평균 AUC-ROC: {avg_auc:.4f}, 평균 F1: {avg_f1:.4f})")
    ...
    print("\n" + "="*50)
    print("전체 데이터셋 VAE(Augmented) 평가 완료!")
    print(f"- 결과 저장 경로: {CSV_OUT_PATH}")
    print(f"- 성공적으로 평가된 데이터셋 수: {len(df)}")
    print(f"- 전체 VAE(Augmented) 평균 AUC-ROC: {df['auc_roc'].mean():.4f}")
    print(f"- 전체 VAE(Augmented) 평균 AUC-PR : {df['auc_pr'].mean():.4f}")
    print(f"- 전체 VAE(Augmented) 평균 F1-Score: {df['unsupervised_f1'].mean():.4f}")
    print("="*50)
