...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_evt.csv"
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001):
    try:
        ...
        # Choose 90th percentile as the tail threshold t
        ...
        if Nt > 10:  # GPD fit requires a minimum number of samples to converge
            try:
                c_fit, loc_fit, scale_fit = scipy.stats.genpareto.fit(excesses, floc=0)
                prob_excess = 1.0 - (q * n / Nt)
                
                # Bound check for probability
                if prob_excess < 0:
                    prob_excess = 0.98
                ...
                thresh_evt = thresh_adaptive  # Fallback to skewness-based if fit errors
        else:
            thresh_evt = thresh_adaptive  # Fallback if too few excesses
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '극값 이론(EVT-POT) 임계치 VAE' 평가를 시작합니다.")
    ...
    for idx, name in enumerate(dataset_names, 1):
        ...
        if idx % 50 == 0 or idx == total_datasets:
            ...
            if valid_results:
                ...
                print(f"[{idx:4d}/{total_datasets:4d}] 완료... (현재 {len(valid_results)}개 - AUC-ROC: {avg_auc:.4f}, Adaptive F1: {avg_adap:.4f}, EVT F1: {avg_evt:.4f})")
    ...
    print("\n" + "="*50)
    print("극값 이론 (EVT-POT) 임계치 벤치마크 완료!")
