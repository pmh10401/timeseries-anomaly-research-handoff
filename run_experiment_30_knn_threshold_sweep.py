...
DETAIL_OUT_PATH = f"{DATA_DIR}/experiment_30_knn_threshold_sweep_results.csv"
SUMMARY_OUT_PATH = f"{DATA_DIR}/experiment_30_knn_threshold_sweep_summary.csv"
LOG_PATH = f"{DATA_DIR}/experiment_30_knn_threshold_sweep.log"
...
WORKERS = env_int("KNN_THRESHOLD_SWEEP_WORKERS", 4)
CONFIG_NAME = "rocket_256_knn5"
NUM_KERNELS = 256
KNN_NEIGHBORS = 5

THRESHOLD_METHODS = [
    ("empirical_q_0025", "empirical_quantile", {"q": 0.0025}),
    ("empirical_q_005", "empirical_quantile", {"q": 0.005}),
    ("empirical_q_0075", "empirical_quantile", {"q": 0.0075}),
    ("empirical_q_010", "empirical_quantile", {"q": 0.01}),
    ("empirical_q_015", "empirical_quantile", {"q": 0.015}),
    ("empirical_q_020", "empirical_quantile", {"q": 0.02}),
    ("empirical_q_030", "empirical_quantile", {"q": 0.03}),
    ("empirical_q_050", "empirical_quantile", {"q": 0.05}),
    ("count_cap_0", "count_cap", {"cap": 0}),
    ("count_cap_1", "count_cap", {"cap": 1}),
    ("count_cap_2", "count_cap", {"cap": 2}),
    ("count_cap_1pct", "count_cap_rate", {"rate": 0.01}),
    ("count_cap_2pct", "count_cap_rate", {"rate": 0.02}),
]


LOG_FORMAT = "%(asctime)s [%(levelname)s] %(message)s"
logger = logging.getLogger("Experiment30KnnThresholdSweep")
...
def empirical_threshold(train_scores, q):
    return float(np.percentile(train_scores, 100.0 * (1.0 - q)))


def count_cap_threshold(train_scores, cap):
    train_scores = clean_scores(train_scores)
    cap = max(0, min(int(cap), len(train_scores) - 1))
    return float(np.sort(train_scores)[len(train_scores) - cap - 1])


def threshold_for_method(train_scores, method_family, params):
    train_scores = clean_scores(train_scores)
    if method_family == "empirical_quantile":
        q = float(params["q"])
        return empirical_threshold(train_scores, q), q, None
    if method_family == "count_cap":
        cap = int(params["cap"])
        return count_cap_threshold(train_scores, cap), cap / max(1, len(train_scores)), cap
    if method_family == "count_cap_rate":
        cap = int(np.floor(float(params["rate"]) * len(train_scores)))
        return count_cap_threshold(train_scores, cap), cap / max(1, len(train_scores)), cap
    raise ValueError(f"Unknown threshold method family: {method_family}")
...
def run_dataset(dataset_name):
    ...
    train_features, test_features = rocket_feature_pair(X_train, X_test, seq_len, NUM_KERNELS)
    train_scores, test_scores = knn_score_pair(train_features, test_features, KNN_NEIGHBORS)
    metrics = score_metrics(y_test, test_scores)
    rows = []
    for method_name, method_family, params in THRESHOLD_METHODS:
        threshold, q_effective, cap_target = threshold_for_method(train_scores, method_family, params)
        ...
        rows.append(
            {
                ...
                "threshold_family": method_family,
                "sequence_length": seq_len,
                "test_size": len(y_test),
                "anomaly_count": int(np.sum(y_test)),
                "train_score_count": len(train_scores),
                "q_effective": q_effective,
                "cap_target": "" if cap_target is None else cap_target,
                "threshold": threshold,
...
def summarize(rows):
    summary = []
    keys = sorted({row["threshold_method"] for row in rows})
    for method in keys:
        subset = [row for row in rows if row["threshold_method"] == method]
        ...
        summary.append(
            {
                ...
                "threshold_family": subset[0]["threshold_family"],
...
def main():
    ...
    logger.info(
        "Starting Experiment 30 KNN threshold sweep on %d datasets with %d workers.",
        len(dataset_names),
        WORKERS,
    )
    detail_rows = []
    ...
    with ProcessPoolExecutor(max_workers=WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            if completed % 25 == 0 or completed == len(dataset_names):
                ...
                if best:
                    logger.info(
                        "Progress: [%4d/%4d] rows=%d | best=%s meanF1=%.4f medianF1=%.4f trainEx=%.4f pred=%.2f",
                        completed,
                        len(dataset_names),
                        len(detail_rows),
    ...
    logger.info("Experiment 30 KNN threshold sweep finished.")
