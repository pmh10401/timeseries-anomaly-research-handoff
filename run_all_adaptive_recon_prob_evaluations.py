...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_recon_prob.csv"
...
class AdaptiveReconProbVAE(nn.Module):
    def __init__(self, latent_dim=128, seq_len=152):
        super(AdaptiveReconProbVAE, self).__init__()
        ...
        # Build Decoder ConvTranspose layers dynamically (Shared backbone)
        dec_layers = []
        ...
        self.dec_shared = nn.Sequential(*dec_layers) if len(dec_layers) > 0 else nn.Identity()
        
        # Branching Heads (mu and logvar)
        # For layers = 1, self.trans_channels is [16, 1]. Shared backbone is Identity.
        # Mu and logvar heads map from 16 to 1.
        last_in_channel = self.trans_channels[-2]
        self.dec_head_mu = nn.ConvTranspose1d(
            in_channels=last_in_channel,
            out_channels=1,
            kernel_size=trans_kernel,
            stride=2,
            padding=self.padding
    ...
    def decode(self, z, target_len):
        ...
        h_shared = self.dec_shared(h)
        
        mu = self.dec_head_mu(h_shared)
        logvar = self.dec_head_logvar(h_shared)
        
        if mu.size(2) != target_len:
            mu = nn.functional.interpolate(mu, size=target_len, mode='linear', align_corners=False)
            logvar = nn.functional.interpolate(logvar, size=target_len, mode='linear', align_corners=False)
            
        # Clamp logvar to prevent standard deviation collapsing to zero (Min std = 0.01)
        logvar = torch.clamp(logvar, min=-9.21, max=5.0)
        return mu, logvar
    ...
    try:
        ...
        for epoch in range(epochs):
            for batch in train_loader:
                ...
                # Reconstruction Loss using Gaussian NLL
                # NLL = 0.5 * logvar + 0.5 * log(2*pi) + (x - mu)^2 / (2 * exp(logvar))
                nll = 0.5 * recon_logvar + 0.5 * np.log(2 * np.pi) + ((x_batch - recon_mu) ** 2) / (2 * torch.exp(recon_logvar))
                recon_loss = nll.mean()
        ...
        # Evaluate TRAIN (on original training data using Monte Carlo Gaussian NLL)
        model.eval()
        train_errors = []
        M = 50 # 50 Monte Carlo samples
        ...
        with torch.no_grad():
            ...
            for batch in train_loader_eval:
                ...
                # MC Sampling for Reconstruction Probability (NLL)
                batch_nll_sum = torch.zeros(x_batch.size(0), device=device)
                for _ in range(M):
                    z = model.reparameterize(mu, logvar)
                    recon_mu, recon_logvar = model.decode(z, x_batch.size(2))
                    
                    nll = 0.5 * recon_logvar + 0.5 * np.log(2 * np.pi) + ((x_batch - recon_mu) ** 2) / (2 * torch.exp(recon_logvar))
                    batch_nll_sum += nll.mean(dim=(1, 2))
                    
                batch_nll_avg = batch_nll_sum / M
                
                # Add KL Divergence component
                kl_errs = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp(), dim=1)
                scores = batch_nll_avg + beta * kl_errs
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '적응형 복원 확률 VAE' 평가를 시작합니다.")
    ...
    print("\n" + "="*50)
    print("적응형 복원 확률 Conv-VAE 벤치마크 완료!")
