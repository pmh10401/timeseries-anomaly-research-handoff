...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_cnn_contrastive.csv"
...
def augment_single_series(x):
    """
    Augment a single time-series sample (Jittering, Scaling, or both)
    """
    L = len(x)
    aug_x = x.copy()
...
def prepare_contrastive_dataset(X, target_size=500):
    N, L = X.shape
    # If N is small, duplicate to meet target_size
    if N < target_size:
    ...
    # Generate 1:1 paired augmented samples
    X_aug = np.array([augment_single_series(x) for x in X_orig])
    return X_orig, X_aug

def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001, gamma=0.1):
    try:
        ...
        # Prepare 1:1 paired original and augmented training batches
        ...
        # Train with Latent Contrastive Loss
        model.train()
        for epoch in range(epochs):
            for batch in train_loader:
                ...
                # 3. 🧪 [실험 1] 잠재 대조 손실 (Contrastive Latent Loss)
                # Cosine Similarity를 최대화하도록 유도 (즉, 1.0 - mean(similarity) 최소화)
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '실험 1: 잠재 대조 학습 VAE' 평가를 시작합니다.")
