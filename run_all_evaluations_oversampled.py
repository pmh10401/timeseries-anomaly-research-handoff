...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/autoencoder_results_oversampled.csv"
...
def run_evaluation(dataset_name, epochs=10, batch_size=128):
    try:
        ...
        # Duplicate/Oversample TRAIN to at least 500 if smaller
        if len(X_train) < 500:
        ...
        train_errors = []
        with torch.no_grad():
            # Only evaluate on the first original_train_size samples to prevent duplicates bias in threshold
...
def main():
    ...
    print(f"총 {total_datasets}개의 데이터셋에 대해 '학습 데이터 500개 이상 중복(Oversampled)' Conv-AE 평가를 시작합니다.")
