...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_stft.csv"
...
def compute_stft_magnitude(x, seq_len):
    ...
    # STFT requires 2D [batch, seq_len]
    x_2d = x.squeeze(1)
...
def compute_sample_spectral_loss(x, x_recon, seq_len):
    ...
    # Compute sample-wise linear loss (Frobenius norm over dim=(1,2))
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001, lambda_spec=0.5):
    try:
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '주파수 도메인 STFT 복원 VAE' 평가를 시작합니다.")
    ...
    print("\n" + "="*50)
    print("주파수 도메인 STFT VAE 벤치마크 완료!")
