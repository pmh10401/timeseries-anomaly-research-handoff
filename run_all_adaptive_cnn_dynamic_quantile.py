...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_cnn_dynamic_quantile.csv"
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001):
    try:
        ...
        # 🧪 [실험 4] 분위수 비례 스케일링 (Dynamic Quantile Selection)
        # q_target = max(0.001, 1.0 / N_test)
        n_test_samples = len(X_test)
        q_target = max(0.001, 1.0 / n_test_samples)
        
        # 1. Dynamic Percentile (100 * (1 - q))
        thresh_percentile = np.percentile(train_errors, 100 * (1.0 - q_target))
        
        # 2. Skewness-based Adaptive Thresholding (scaled target q)
        ...
        # 3. Extreme Value Theory (EVT) POT GPD fit (scaled target q)
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '실험 4: 분위수 비례 스케일링 VAE' 평가를 시작합니다.")
