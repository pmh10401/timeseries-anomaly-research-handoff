...
logging.basicConfig(
    ...
    format=LOG_FORMAT,
    handlers=[
        logging.FileHandler("/Users/minho/Documents/Dataset/true_infonce_multi_aug_run.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("TrueInfoNCEMultiAugVAE")

# DB Path
DB_PATH = "/Users/minho/Documents/Dataset/univariate_ts.db"
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_cnn_true_infonce_multi_aug.csv"
...
def true_infonce_loss(z_orig, z_aug, temperature=0.2):
    z_orig = nn.functional.normalize(z_orig, dim=1)
    z_aug = nn.functional.normalize(z_aug, dim=1)
    logits = torch.matmul(z_orig, z_aug.T) / temperature
    labels = torch.arange(z_orig.size(0), device=z_orig.device)
    loss_orig_to_aug = nn.functional.cross_entropy(logits, labels)
    loss_aug_to_orig = nn.functional.cross_entropy(logits.T, labels)
    return 0.5 * (loss_orig_to_aug + loss_aug_to_orig)
...
def load_dataset_data(dataset_name):
    ...
    X_train = [sanitize_series(np.frombuffer(row[0], dtype=np.float32)) for row in train_rows]
    ...
    X_train = align_series_lengths(X_train, target_len)
    X_test = align_series_lengths(X_test, target_len)
    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                # Symmetric InfoNCE: matched pairs are positives, the rest of the batch are negatives.
                contrastive_loss = true_infonce_loss(z_orig, z_aug, temperature=0.2)
...
def main():
    ...
    logger.info(f"Starting True InfoNCE Multi-Augmentation VAE on {total_datasets} datasets.")
    ...
    for idx, name in enumerate(dataset_names, 1):
        ...
        if idx % 50 == 0 or idx == total_datasets:
            ...
            save_results_csv(valid_results, CSV_OUT_PATH)
            if valid_results:
    ...
    valid_results = [r for r in results if r is not None]
    save_results_csv(valid_results, CSV_OUT_PATH)
    logger.info("True InfoNCE Multi-Augmentation VAE Finished!")
    if valid_results:
        logger.info(f"- Mean F1 (EVT-POT): {np.mean([r['f1_evt'] for r in valid_results]):.4f}")
