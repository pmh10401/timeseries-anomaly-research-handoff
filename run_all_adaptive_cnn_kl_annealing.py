...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_cnn_kl_annealing.csv"
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, max_beta=0.001):
    try:
        ...
        for epoch in range(epochs):
            # 🧪 [실험 3] KL Annealing (0.0에서 max_beta까지 선형 스케줄링)
            # 7에폭까지 점진적 상승 후, 남은 3에폭은 max_beta로 고정 학습
        ...
        # Evaluate TRAIN (on original training data using max_beta for final score)
        model.eval()
        train_errors = []
        with torch.no_grad():
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '실험 3: KL Annealing VAE' 평가를 시작합니다.")
    ...
    print("\n" + "="*50)
    print("KL Annealing VAE 벤치마크 완료!")
