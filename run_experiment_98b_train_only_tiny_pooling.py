...
EXPERIMENT_ID = "experiment_98b_train_only_tiny_pooling"
...
def load_target_names(exp93_rows):
    pig_families = {"PigAirwayPressure", "PigArtPressure", "PigCVP"}
    return {
        name
        for name, row in exp93_rows.items()
        if int(as_float(row.get("train_normal_count"), 0)) <= 10 or row.get("family") in pig_families
    }
...
def run_experiment(dataset_limit=None):
    ...
    targets = load_target_names(exp93)
    if dataset_limit:
