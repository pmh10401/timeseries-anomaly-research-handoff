...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_periodicity_hybrid.csv"
...
def check_dataset_periodicity(X):
    # Determine periodicity using ACF peak detection
    n_samples, seq_len = X.shape
    if seq_len < 15: # Too short to establish periodicity
        return False
        
    sample_periodic_flags = []
    # Sample up to 50 series to optimize runtime
    ...
    for idx in sample_indices:
        ...
        # Look for local peaks excluding lags close to 0 (we require lag >= 5 to avoid overlap)
        peaks = []
        ...
        is_periodic = False
        if peaks:
            best_peak = max(peaks, key=lambda v: v[1])
            if best_peak[1] > 0.40: # Prominent ACF peak
                is_periodic = True
        sample_periodic_flags.append(is_periodic)
        
    # Periodic if >= 50% of sampled series exhibit periodic ACF peaks
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001):
    try:
        ...
        # 1. Classify dataset periodicity
        is_periodic = check_dataset_periodicity(X_train)
        
        # Set spectral loss weight dynamically
        ...
        for epoch in range(epochs):
            for batch in train_loader:
                ...
                # Spectral loss (only calculated if lambda_spec > 0)
                if lambda_spec > 0:
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '주기성 분기 하이브리드 VAE' 평가를 시작합니다.")
    ...
    print("\n" + "="*50)
    print("주기성 분기형 하이브리드 VAE 벤치마크 완료!")
