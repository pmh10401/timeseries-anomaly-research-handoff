...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_recon_prob.csv"
...
class ConvReconProbVAE(nn.Module):
    def __init__(self, latent_dim=128, seq_len=152):
        super(ConvReconProbVAE, self).__init__()
        ...
        # Decoder Conv outputs 2 channels (Channel 0: Mean, Channel 1: Log-variance)
        self.dec_conv = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.ConvTranspose1d(16, 2, kernel_size=16, stride=2, padding=7), # Outputs 2 channels
        )
    ...
    def decode(self, z, target_len):
        ...
        x_out = self.dec_conv(h)
        if x_out.size(2) != target_len:
            x_out = nn.functional.interpolate(x_out, size=target_len, mode='linear', align_corners=False)
            
        x_mean = x_out[:, 0:1, :]
        x_logvar = x_out[:, 1:2, :]
        
        # Clamp logvar to prevent division by zero or NaN issues
    ...
    def forward(self, x):
        ...
        x_mean, x_logvar = self.decode(z, x.size(2))
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001, M_samples=10):
    try:
        ...
        # Model
        model = ConvReconProbVAE(latent_dim=128, seq_len=X_train.shape[1]).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
        
        # Train VAE with NLL loss (Reconstruction Probability Loss)
        model.train()
        for epoch in range(epochs):
            for batch in train_loader:
                x_batch = batch[0].to(device)
                optimizer.zero_grad()
                recon_mean, recon_logvar, mu, logvar = model(x_batch)
                
                # NLL Loss (Gaussian distribution assumption)
                var_x = torch.exp(recon_logvar)
                recon_loss = 0.5 * torch.mean(recon_logvar + ((x_batch - recon_mean) ** 2) / var_x)
        ...
        # Evaluate Reconstruction Probability on TRAIN (for threshold calibration)
        model.eval()
        train_errors = []
        with torch.no_grad():
            ...
            for batch in train_loader_eval:
                ...
                # MC sampling for reconstruction probability
                n_samples_batch = x_batch.size(0)
                batch_nll_sum = torch.zeros(n_samples_batch, device=device)
                
                for _ in range(M_samples):
                    z = model.reparameterize(mu, logvar)
                    recon_mean, recon_logvar = model.decode(z, x_batch.size(2))
                    var_x = torch.exp(recon_logvar)
                    
                    # Compute negative log-likelihood per sample
                    nll = 0.5 * (recon_logvar + ((x_batch - recon_mean) ** 2) / var_x).mean(dim=2).squeeze(1)
                    batch_nll_sum += nll
                    
                kl_errs = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp(), dim=1)
                mean_nll = batch_nll_sum / M_samples
                scores = mean_nll + beta * kl_errs
                train_errors.extend(scores.cpu().numpy())
        train_errors = np.array(train_errors)
        
        # Evaluate Reconstruction Probability on TEST
        test_errors = []
        with torch.no_grad():
        ...
        # Skewness-based Adaptive Thresholding
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '복원 확률(Reconstruction Probability) VAE' 평가를 시작합니다.")
    ...
    for idx, name in enumerate(dataset_names, 1):
        ...
        if idx % 50 == 0 or idx == total_datasets:
            ...
            if valid_results:
                ...
                print(f"[{idx:4d}/{total_datasets:4d}] 진행 완료... (현재 완료된 {len(valid_results)}개 VAE - AUC-ROC: {avg_auc:.4f}, Adaptive F1: {avg_adap:.4f})")
