...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_online_dynamic.csv"
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001):
    try:
        ...
        # 3. Extreme Value Theory (EVT) Peaks-Over-Threshold (POT) - GPD fit (Static Reference)
        ...
        # 4. [NEW] Online Dynamic Thresholding + EMA Filter
        # Parameters Configuration
        W = 50  # window size
        if len(anomaly_scores) < W:
            W = max(5, len(anomaly_scores) // 4)
            
        alpha = 0.15      # EMA smoothing factor
        k_margin = 1.0    # Bollinger standard deviation scale
        gamma_scale = 0.3 # Covariate scaling factor
        
        mu_train = np.mean(train_errors)
        
        # Initialize FIFO buffer for local window with train tail errors to avoid cold start issues
        local_window = list(train_errors[-W:]) if len(train_errors) >= W else list(train_errors)
        if len(local_window) < W:
            local_window = [mu_train] * (W - len(local_window)) + local_window
            
        dynamic_predictions = []
        ema_scores = []
        current_ema = mu_train # Start EMA with average train error for stability
        
        for s_t in anomaly_scores:
            # A. EMA Filter to suppress single-point noise
            current_ema = alpha * s_t + (1.0 - alpha) * current_ema
            ema_scores.append(current_ema)
            
            # B. Local window statistics
            mu_local = np.mean(local_window)
            sigma_local = np.std(local_window)
            
            # C. Dynamic Bollinger Margin calculation
            z_t = thresh_evt + k_margin * sigma_local
            
            # D. Covariate Scale Alignment
            z_final = z_t * (gamma_scale * (mu_local / mu_train) + (1.0 - gamma_scale))
            
            # E. Anomaly Decision
            pred = 1 if current_ema > z_final else 0
            dynamic_predictions.append(pred)
            
            # F. Update FIFO Buffer
            local_window.pop(0)
            local_window.append(s_t)
        ...
        f1_online_dynamic = f1_score(y_test, dynamic_predictions, zero_division=0)
        ...
        return {
            ...
            "f1_evt": f1_evt,
            "f1_online_dynamic": f1_online_dynamic,
            "oracle_f1": best_f1
        }
    except Exception as e:
        return None
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '온라인 동적 임계값 및 EMA 필터 VAE' 평가를 시작합니다.")
    ...
    for idx, name in enumerate(dataset_names, 1):
        ...
        if idx % 50 == 0 or idx == total_datasets:
            ...
            if valid_results:
                ...
                avg_online = np.mean([r['f1_online_dynamic'] for r in valid_results])
                print(f"[{idx:4d}/{total_datasets:4d}] 완료... (현재 {len(valid_results)}개 - AUC-ROC: {avg_auc:.4f}, Adaptive F1: {avg_adap:.4f}, EVT F1: {avg_evt:.4f}, Online Dynamic F1: {avg_online:.4f})")
    ...
    print(f"- 온라인 동적 임계치 평균 F1 : {df['f1_online_dynamic'].mean():.4f} 🚀")
