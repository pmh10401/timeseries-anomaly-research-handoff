...
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_transformer.csv"
...
class AdaptiveTransformerVAE(nn.Module):
    def __init__(self, latent_dim=128, seq_len=152):
        super(AdaptiveTransformerVAE, self).__init__()
        ...
        # 🧪 [실험 2] Encoder Self-Attention Layer
        # Num heads is bounded to last_channel constraints
        n_heads_enc = 1
        if self.last_channel >= 4:
            n_heads_enc = 4
        self.enc_attention = nn.MultiheadAttention(embed_dim=self.last_channel, num_heads=n_heads_enc)
        ...
        # 🧪 [실험 2] Decoder Self-Attention Layer
        self.dec_attention = nn.MultiheadAttention(embed_dim=self.last_channel, num_heads=n_heads_enc)
    ...
    def encode(self, x):
        h = self.enc_conv(x) # (B, C, L_conv)
        
        # Permute for Self-Attention (Seq_len, Batch_size, Embed_dim)
        h = h.permute(2, 0, 1)
        h_attn, _ = self.enc_attention(h, h, h)
        
        # Permute back to (Batch_size, Embed_dim, Seq_len)
        h = h_attn.permute(1, 2, 0).contiguous()
        h = h.view(h.size(0), -1)
    ...
    def decode(self, z, target_len):
        h = self.dec_fc(z)
        h = h.view(h.size(0), self.last_channel, self.conv_out_len) # (B, C, L_conv)
        
        # Permute for Self-Attention
        h = h.permute(2, 0, 1)
        h_attn, _ = self.dec_attention(h, h, h)
...
def run_evaluation(dataset_name, epochs=10, batch_size=128, beta=0.001):
    try:
        ...
        # Model (Adaptive Transformer-VAE)
        model = AdaptiveTransformerVAE(latent_dim=128, seq_len=seq_len).to(device)
...
def main():
    ...
    print(f"총 {total_datasets}개의 전체 데이터셋에 대해 '실험 2: Transformer 융합 VAE' 평가를 시작합니다.")
    ...
    print("\n" + "="*50)
    print("Transformer 융합 VAE 벤치마크 완료!")
