...
logging.basicConfig(
    ...
    format=LOG_FORMAT,
    handlers=[
        logging.FileHandler("/Users/minho/Documents/Dataset/fused_sota_run.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("FusedSotaVAE")

# DB Path
DB_PATH = "/Users/minho/Documents/Dataset/univariate_ts.db"
CSV_OUT_PATH = "/Users/minho/Documents/Dataset/vae_results_adaptive_cnn_fused_sota.csv"
...
def run_evaluation(dataset_name, epochs=10, batch_size=128):
    try:
        ...
        if len(X_train) == 0 or len(X_test) == 0:
            logger.warning(f"Dataset {dataset_name} has empty train or test set.")
            return None
            
        original_train_size = len(X_train)
        seq_len = X_train.shape[1]
        
        # 🧪 Dynamically schedule hyperparameters based on metadata
        # Longer sequence lengths get smaller KL weight to prevent KLD collapse
        max_beta = 0.15 / seq_len
        # Smaller train sets get larger contrastive learning weight to avoid overfitting
        gamma = 0.8 / np.log(original_train_size + 2)
        
        logger.info(f"Dataset: {dataset_name} | Length: {seq_len} | Train size: {original_train_size}")
        logger.info(f"Scheduled parameters: max_beta={max_beta:.6f}, gamma={gamma:.4f}")
        
        X_train = z_normalize(X_train)
        X_test = z_normalize(X_test)
        
        # Prepare 1:1 paired contrastive training data
        ...
        # Train with Fused Strategy (KL Annealing + Contrastive Latent)
        model.train()
        for epoch in range(epochs):
            # 🧪 KL Weight Annealing (Dynamic scaling up to max_beta)
            ...
            avg_contrast = epoch_contrast / steps
            logger.debug(
                f"[{dataset_name}] Epoch {epoch+1:2d} | Loss: {avg_loss:.5f} (Recon: {avg_recon:.5f}, "
                f"KL: {avg_kl:.5f} (beta={current_beta:.6f}), Contrast: {avg_contrast:.5f})"
            )
                
        # Evaluate TRAIN (on original training data)
        model.eval()
        train_errors = []
        with torch.no_grad():
        ...
        # 3. Extreme Value Theory (EVT) POT GPD fit (scaled q)
        ...
        logger.info(
            f"[{dataset_name}] Done | AUC-ROC: {auc_roc:.4f} | EVT F1: {f1_evt:.4f} | Oracle F1: {best_f1:.4f}"
        )
...
def main():
    ...
    logger.info("="*50)
    logger.info(f"Starting Fused SOTA VAE evaluation on {total_datasets} datasets.")
    logger.info(f"Results will be saved to: {CSV_OUT_PATH}")
    logger.info("="*50)
    ...
    for idx, name in enumerate(dataset_names, 1):
        ...
        if idx % 50 == 0 or idx == total_datasets:
            ...
            if valid_results:
                ...
                logger.info(
                    f"Progress: [{idx:4d}/{total_datasets:4d}] | CumAvg AUC-ROC: {avg_auc:.4f} | CumAvg EVT F1: {avg_evt:.4f}"
                )
    ...
    logger.info("\n" + "="*50)
    logger.info("Fused SOTA VAE Benchmark Finished!")
    logger.info(f"- Output CSV: {CSV_OUT_PATH}")
    logger.info(f"- Evaluated datasets: {len(df)}")
    logger.info(f"- Mean AUC-ROC     : {df['auc_roc'].mean():.4f}")
    logger.info(f"- Mean AUC-PR      : {df['auc_pr'].mean():.4f}")
    logger.info(f"- Mean F1 (Percentile): {df['f1_percentile'].mean():.4f}")
    logger.info(f"- Mean F1 (Adaptive)  : {df['f1_adaptive'].mean():.4f}")
    logger.info(f"- Mean F1 (EVT-POT)   : {df['f1_evt'].mean():.4f}")
    logger.info(f"- Mean F1 (Oracle)    : {df['oracle_f1'].mean():.4f}")
    logger.info("="*50)
