...
DETAIL_OUT_PATH = f"{DATA_DIR}/experiment_31_knn_operational_budget_sweep_results.csv"
SUMMARY_OUT_PATH = f"{DATA_DIR}/experiment_31_knn_operational_budget_sweep_summary.csv"
LOG_PATH = f"{DATA_DIR}/experiment_31_knn_operational_budget_sweep.log"
...
WORKERS = env_int("KNN_OPERATIONAL_BUDGET_WORKERS", 4)
...
THRESHOLD_METHODS = [
    ...
    ("count_cap_0_5pct", "count_cap_rate", {"rate": 0.005}),
    ("count_cap_1pct", "count_cap_rate", {"rate": 0.01}),
    ("count_cap_1_5pct", "count_cap_rate", {"rate": 0.015}),
    ("count_cap_2pct", "count_cap_rate", {"rate": 0.02}),
    ("count_cap_2_5pct", "count_cap_rate", {"rate": 0.025}),
    ("count_cap_3pct", "count_cap_rate", {"rate": 0.03}),
    ("count_cap_4pct", "count_cap_rate", {"rate": 0.04}),
    ("count_cap_5pct", "count_cap_rate", {"rate": 0.05}),
    ("empirical_q_020_reference", "empirical_quantile", {"q": 0.02}),
]


LOG_FORMAT = "%(asctime)s [%(levelname)s] %(message)s"
logger = logging.getLogger("Experiment31KnnOperationalBudgetSweep")
...
def summarize(rows):
    summary = []
    for method in sorted({row["threshold_method"] for row in rows}):
...
def main():
    ...
    logger.info(
        "Starting Experiment 31 KNN operational budget sweep on %d datasets with %d workers.",
        len(dataset_names),
        WORKERS,
    )
    detail_rows = []
    ...
    logger.info("Experiment 31 KNN operational budget sweep finished.")
