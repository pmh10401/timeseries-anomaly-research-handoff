...
EXPERIMENT_ID = "experiment_94b_corrected_nonpos_rank_consensus"
...
WORKERS = int(os.environ.get("EXP94B_WORKERS", "4"))
    ...
    out.update(
        {
            ...
            "score_family": "corrected_nonpos_rank_consensus",
...
def candidate_context(dataset_name, exp87_rows, base_indices):
    record, y_test, bundles = load_candidate_predictions(
        dataset_name,
        threshold_rates=CALIBRATION_PROFILES["relaxed_15pct"],
    )
    rocket_order = bundle_order(bundles["rocket_exp40"], 32)
...
def top_candidate(scored, base_indices, min_support, max_best_rank, min_gain):
    base_scores = [scored.get(idx, {}).get("score", 0.0) for idx in base_indices]
    base_best = max(base_scores or [0.0])
...
def choose_rows_for_dataset(args):
    ...
    record, y_test, bundles, scored = candidate_context(dataset_name, exp87_rows, base_indices)
    ...
    configs = [
        ("corrected_replace_strict", 3, 3, 0.10, 1, 6, 0.35),
        ("corrected_replace_balanced", 2, 3, 0.12, 1, 6, 0.30),
        ("corrected_replace_margin", 2, 2, 0.20, 1, 5, 0.40),
        ("corrected_replace_or_add_strict", 4, 2, 0.18, 1, 6, 0.35),
    ]
    for name, min_support, max_rank, min_gain, weak_support, weak_rank, weak_score in configs:
        ...
        if not tiny_train and sparse and cand is not None:
            ...
            elif name.startswith("corrected_replace_or_add") and not base_indices:
                chosen.add(cand)
        ...
        rows.append(
            row_with_metrics(
                ...
                name,
                chosen,
                "Exp94b corrected non-position rank consensus; base alert included in scoring pool",
                {
