...
**Files:** Create root `*.py`, `scratch/test_*.py`, `docs/source/`, `results/`, `artifacts/`, and `.gitignore`.

- [x] Copy root Python scripts and available scratch tests at their original paths, excluding bytecode and virtual environments.
- [x] Copy all Markdown/Mermaid documentation plus reviewable CSV/JSON/figure artifacts, excluding duplicate archives and logs.
- [x] Enforce the deny-list: DBs, raw data, checkpoints, JSONL resume files, environments, and files over 90 MB.
...
- [x] Export the full `datasets` table ordered by name.
- [x] Record schema, aggregate counts, evaluation exclusions, normal-only training policy, and limits on industrial interpretation.

### Task 3: Add entry points and traceability

**Files:** Create `README.md`, `MANIFEST.csv`, `MANIFEST.md`, and `REPRODUCIBILITY.md`.

- [x] Write a plain-language research map, operational objective, Exp137 evidence, and GPT handoff steps.
- [x] Generate SHA256 entries for each included source/result artifact and record excluded classes.

### Task 4: Validate and publish

- [x] Scan staged text for credential values and confirm every large file remains below the GitHub limit.
- [x] Run source compilation, selected tests, and a staged-file review. Run git fsck after committing the export.
